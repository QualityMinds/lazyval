...
// currently not supported by Github (maybe the Reducer can handle this later)
:toc:
:toclevels: 2
...
https://martinfowler.com/bliki/ValueObject.html[value-types]. The initial idea was inspired by
Scalas https://www.scala-lang.org/api/current/scala/AnyVal.html[Anyval]; even though this works
completely differently, it allows you to easily define a wrapper-type which captures the domain's
intent.

In the usual Java Enterprise projects, it is generally a good idea to model types instead of relying
purely on primitives.
...
As an example, assuming a regular project (Quarkus, Spring Boot) with Mapstruct and JPA,
for each custom value-type it is necessary to provide a https://www.baeldung.com/jpa-attribute-converters[JPA AttributeConverter]
as well as a Mapstruct Mapper definition to wrap/unpack the type.

*This is what Lazyval is doing: depending on your classpath, it will generate*:

* *a single Mapstruct-Mapper with mappings for all defined value-types*

* *a JPA-AttributeConverter for each value-type*

NOTE: mappers for other database systems like Cassandra, Mongo, should be fairly easy to add in the future.
...
** for regular classes, the class and the value should be final; otherwise a warning is issued

* might provide a single factory method which takes precedence over constructors for object creation

* must be immutable (currently not enforced)
