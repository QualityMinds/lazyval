...
import de.qualityminds.lazyval.testkit.dependencies.ClasspathDependency;
...
public class ToolchainSetup {
    ...
    private static final InternalModuleDependency CORE_MODULE = new InternalModuleDependency("../core");
    ...
    private static URLClassLoader createSymbolProcessorClassloader(
            ...
            Scenario.Descriptor scenarioDescriptor) {
        var additionalClasspath = new ArrayList<File>();
        additionalClasspath.addAll(CORE_MODULE.resolve().toSet());
        scenarioDescriptor.dependencies().stream()
                .map(ClasspathDependency::resolve)
                .forEach(x -> additionalClasspath.addAll(x.toSet()));
    ...
    private static KotlinSymbolProcessing setupKsp2(
        ...
        try {
            var additionalClasspath = new ArrayList<File>();
            additionalClasspath.addAll(CORE_MODULE.resolve().toSet());
            scenarioDescriptor.dependencies().stream().map(ClasspathDependency::resolve).forEach(x -> additionalClasspath.addAll(x.toSet()));
            ...
            compilationUnit.addAll(CORE_MODULE.resolve().toSet());
