 ...
 *
 * Shared by {@code KspSpringDataIT} (generator behaviour) and {@code KspSpringDataConverterRulesIT}
 * (user-converter rules) so a new store is declared exactly once.
 */
class KspSpringDataStores {
    ...
    /**
     * Present only so the generated `@Generated` stamp can be emitted — the KSP generator omits the
     * annotation when it is absent, which {@code KspSpringDataIT} asserts.
     */
    static final Dependency JAKARTA_ANNOTATIONS =
            new Dependency('jakarta.annotation', 'jakarta.annotation-api', '3.0.0')

    /** On the classpath for every store; without a store module this alone generates nothing. */
    static final List<Dependency> SPRING_BASELINE = [
    ...
    static Dependency[] classpathFor(Store store) {
        (store.modules + SPRING_BASELINE + [JAKARTA_ANNOTATIONS]) as Dependency[]
    }

    /** Same classpath, minus the annotation API — used to assert the `@Generated` stamp is optional. */
    static Dependency[] classpathWithoutAnnotationApi(Store store) {
