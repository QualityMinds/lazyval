...
@Title("Kotlin - JakartaEE MongoDB")
@Testcontainers
@Stepwise
class JeeMongoIT extends AbstractLibertyIT {
    ...
    def "should find order by id"() {
        when:
        def foundOrder = mapper.toDomainOrder(orderApi.getOrderByIdMongo(StartupBean.DefaultOrderB.id))
    ...
    def "should find orders by isbn"() {
        when:
        def foundOrders = mapper.toDomainOrder(orderApi.findOrdersByIsbnMongo(StartupBean.DefaultOrderB.isbn.value))
